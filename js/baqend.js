DB.connect('http://ec2-54-164-72-24.compute-1.amazonaws.com:8080', function() {
  var ready = Promise.resolve();
  if(~location.search.indexOf("init=true")) {
    var emf = new baqend.EntityManagerFactory('http://ec2-54-164-72-24.compute-1.amazonaws.com:8080');
    var metamodel = emf.metamodel;

    metamodel.init();

    var newsType = new baqend.metamodel.EntityType("News", metamodel.entity(Object));
    newsType.declaredAttributes.push(new baqend.metamodel.SingularAttribute(newsType, "headline", metamodel.baseType(String)));
    newsType.declaredAttributes.push(new baqend.metamodel.SingularAttribute(newsType, "short", metamodel.baseType(String)));
    newsType.declaredAttributes.push(new baqend.metamodel.SingularAttribute(newsType, "img", metamodel.baseType(String)));
    newsType.declaredAttributes.push(new baqend.metamodel.SingularAttribute(newsType, "tag", metamodel.baseType(String)));
    newsType.declaredAttributes.push(new baqend.metamodel.SingularAttribute(newsType, "date", metamodel.baseType(String)));
    metamodel.addType(newsType);

    var streamType = new baqend.metamodel.EntityType("Stream", metamodel.entity(Object));
    streamType.declaredAttributes.push(new baqend.metamodel.SingularAttribute(newsType, "titel", metamodel.baseType(String)));
    streamType.declaredAttributes.push(new baqend.metamodel.ListAttribute(newsType, "news", newsType));
    metamodel.addType(streamType);


    ready = metamodel.save(true, "WAAublTINO5UbLRmAAAAAQAAAAGva1CWgdR4JdcO5g1Zo0cITBLKzA==");
    //  .then(function() {
    ...
    //  politik.news = baqend.List([p5, p6, p1, p2, p3, p4, p7, p8, p9, p10]);
    //
    //  var wirtschaft = db.Stream();
    //  wirtschaft.id = 'wirtschaft';
    //  wirtschaft.titel = 'Wirtschaft';
    //  wirtschaft.news = baqend.List([w1, w2, w3, w4, w5, w6, w7, w8, w9, w10]);
    //
    //  var kultur = db.Stream();
    //  kultur.id = 'kultur';
    //  kultur.titel = 'Kultur';
    //  kultur.news = baqend.List([k1, k2, k3, k4, k5, k6, k7, k8, k9, k10]);
    //
    //  return Promise.all([politik.save(true, true), wirtschaft.save(true, true), kultur.save(true, true)]);
    //});
  }

  ready.then(function() {
    ['wirtschaft', 'politik', 'kultur'].forEach(function(streamId) {
      DB.Stream.get(streamId, function(stream) {
        stream.news.forEach(function(news) {
          render(stream.titel, DB.News.get(news.id));
        });
      });
    });
  });
